  ...
  <div>
    <div>
      <Modal
        ...
        <div>
          <Form ref="logoadd" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
            <Form-item label="LOGO名称" prop="name">
              <Input type="text" v-model="form.name" placeholder="请输入名称"></Input>
            </Form-item>
            <Form-item label="LOGO" prop="oss_logo_path">
              <Upload
                type="select"
                ref="upImg"
                with-credentials
  ...
  export default {
    ...
    methods: {
      ...
      getResponse(response, file, filelist) {
        this.form.oss_logo_path = response.url;
        if (response.status) {
      ...
      add() {
        this.$refs.logoadd.validate((valid) => {
          if (valid) {
            this.modal_loading = true;
            let data = this.form;
            this.apiPost('admin/siteLogo', data).then((res) => {
              this.handelResponse(res, (data, msg) => {
                this.modal = false;
                this.$parent.getData();
                this.$Message.success(msg);
                this.modal_loading = false;
                this.$refs.logoadd.resetFields();
              }, (data, msg) => {
                this.modal_loading = false;
