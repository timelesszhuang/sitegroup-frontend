  ...
  <div>
    <div>
      <Modal
        ...
        <div>
          <Form ref="logosave" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
            <Form-item label="LOGO名称" prop="name">
              <Input type="text" v-model="form.name" placeholder="请输入名称"></Input>
            </Form-item>
            <Form-item label="LOGO" prop="name">
              <Upload
                type="select"
                ref="upImg"
                with-credentials
        ...
        add() {
          this.$refs.logosave.validate((valid) => {
              if(valid){
                ...
                let data = this.form;
                let id = data.id;
                this.apiPut('admin/siteLogo/'+ id, data).then((res) => {
                  this.handelResponse(res, (data, msg) => {
                    this.modal = false;
                    this.$parent.getData();
                    this.$Message.success(msg);
                    this.modal_loading = false;
                    this.$refs.logosave.resetFields();
                  }, (data, msg) => {
                    this.modal_loading = false;
