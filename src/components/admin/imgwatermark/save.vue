      ...
      <Modal
        ...
        <div>
          <Form ref="icosave" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
            <Form-item label="水印名称" prop="name">
              <Input type="text" v-model="form.name" placeholder="请输入水印名称"></Input>
            </Form-item>
            <Form-item label="IMG" prop="name">
              <Upload
                type="select"
                ref="upImg"
                with-credentials
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      imgshow: function () {
        if (this.form.oss_water_image_path) {
          return true;
        }
        return false;
      }
        ...
        add() {
          this.$refs.icosave.validate((valid) => {
              if(valid){
                ...
                let data = this.form;
                let id = data.id;
                this.apiPut('admin/siteWaterImage/'+ id, data).then((res) => {
