      ...
      >
        ...
        <div>
          ...
          <Form ref="editimg" :model="form" :label-width="90" :rules="AddRule" class="node-add-form">
            <div style="margin-bottom:20px ">
              <Upload
                ...
                :data="addotherdata"
              >
                <Button type="primary" icon="ios-cloud-upload-outline">添加活动图片</Button>
              </Upload>
            </div>
            <Row v-if="is_show">
              <Col span="8">
              <Form-item label="修改活动图片" prop="imgser">
                <Select style="width:120px;" placeholder="活动图片" label-in-value filterable
          ...
          <Alert v-show="!is_show">
            该活动暂时没有图片
            <template slot="desc">请添加活动图片</template>
          </Alert>
        </div>
        <div slot="footer">
        </div>
  ...
  export default {
    data() {
      return {
        ...
        //是不是需要显示修改等操作
        is_show: false,
        action: HOST + 'admin/uploadactivitySerImg',
      }
    },
    created() {
    },
    ...
    methods: {
      ...
      delimg() {
        ...
        this.$Modal.confirm({
          ...
          cancelText: '取消',
          onOk: (index) => {
            _this.apiGet('admin/delImgSer/' + id + '/' + _this.imgIndex).then((res) => {
