      ...
      <Row :gutter="10" type="flex" justify="center" class="code-row-bg rowmain">
        ...
        <div class="homehead">
          <div style="padding-bottom: 10px">
            ...
            <span class="footfont" style="display: inline-block;margin-left:15px;font-size: 25px;color: #FF0000;">{{pv}}</span>
          </div>
      ...
      <Row :gutter="60" style="margin: 10px">
        ...
        <Carousel autoplay v-model="value2">
          <CarouselItem v-for="(item,index) in marketdata" :key="index" @click="show(index)">
            <div @click="show(index)" style="height: 190px;cursor:pointer">
              <img style="display:block;margin: 0 auto" :src=formatter_str(item.img) :alt="item.title" >
            </div>
          <div  style="text-align: center;font-size: 15px;margin-bottom: 10px">{{item.title}}</div>
          </CarouselItem>
        </Carousel>
        </Col>
